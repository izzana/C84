...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    //pode ser usada para decidir se devemos emitir ou devolver o livro. 
    //Além disso, podemos usá-la para verificar se o livro está na biblioteca
  ...
  checkBookAvailability = async bookId => {
    const bookRef = await db //fazendo checagem no bd
      .collection("books")
      .where("book_id", "==", bookId)
      .get();
  ...
  checkStudentEligibilityForBookIssue = async studentId => {
    ...
    var isStudentEligible = ""; //informará a elegibilidade do aluno
    //verificando se o id do aluno não corresponde ou não está presente no BD
    ...
    } else {//será executado quando o ID do aluno existir no BD
      studentRef.docs.map(doc => {
        if (doc.data().number_of_books_issued < 2) {
          isStudentEligible = true;//se pegou menos que 2 livros pode pegar mais
        } else {
          isStudentEligible = false;
          Alert.alert("O aluno não pode retirar mais de 2 livros");
          this.setState({
            bookId: "",
            studentId: ""
          });
  ...
  checkStudentEligibilityForBookReturn = async (bookId, studentId) => {
    //localizando as transações na coleção transaction 
    //que correspondam ao bookId fornecido
    const transactionRef = await db
      .collection("transactions") //coleção que possui os dados do livro que é retirado
      .where("book_id", "==", bookId)
      .limit(1) //retornará apenas a última transação do livro
      .get();
